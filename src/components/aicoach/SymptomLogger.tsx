...
export const SymptomLogger: React.FC<SymptomLoggerProps> = ({ onLogAdded }) => {
    ...
    return (
        <div className="relative bg-white/5 backdrop-blur-xl rounded-2xl shadow-sm border border-white/10 p-5 md:p-8 overflow-hidden">
            ...
            <h3 className="text-sm font-bold uppercase tracking-widest text-slate-300 mb-8 flex items-center relative z-10">
                <Activity className="w-5 h-5 mr-3 text-cyan-400 drop-shadow-[0_0_8px_rgba(34,211,238,0.8)]" />
                Log New Symptom
            </h3>
            ...
            <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
                {/* Symptom Selection */}
                <div>
                    <label className="block text-sm font-bold tracking-wide text-slate-400 mb-4">WHAT ARE YOU EXPERIENCING?</label>
                            ...
                            <button
                                ...
                                className={clsx(
                                    ...
                                    symptom === s
                                        ...
                                        : 'bg-white/5 text-slate-300 border-white/10 hover:border-cyan-500/30 hover:bg-white/10'
                                )}
                            >
                                {s}
                            </button>
                    ...
                    {symptom === 'Other' && (
                        <input
                            ...
                            className="mt-4 w-full px-5 py-3 bg-[#0f172a]/50 text-white border border-white/10 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition-all placeholder:text-slate-500"
                            required
                        />
                    )}
                </div>
                ...
                {/* Severity */}
                <div>
                    <label className="block text-sm font-bold tracking-wide text-slate-400 mb-4">SEVERITY</label>
                    <div className="flex space-x-6 bg-white/5 p-4 rounded-xl border border-white/5">
                        {(['Mild', 'Moderate', 'Severe'] as const).map(level => (
                            <label key={level} className="flex items-center cursor-pointer group">
                                <input
                                    ...
                                    className="w-4 h-4 text-cyan-500 bg-white/10 border-white/20 focus:ring-cyan-500 focus:ring-offset-slate-900"
                                />
                                <span className={clsx(
                                    'ml-3 text-sm font-bold tracking-wide transition-colors',
                                    severity === level ? (
                                        level === 'Severe' ? 'text-rose-400 drop-shadow-[0_0_8px_rgba(244,63,94,0.5)]' :
                                            level === 'Moderate' ? 'text-amber-400 drop-shadow-[0_0_8px_rgba(251,191,36,0.5)]' : 'text-emerald-400 drop-shadow-[0_0_8px_rgba(52,211,153,0.5)]'
                ...
                {/* Notes */}
                <div>
                    <label className="block text-sm font-bold tracking-wide text-slate-400 mb-4">ADDITIONAL NOTES (OPTIONAL)</label>
                    <textarea
                        rows={3}
                        placeholder="E.g., Started after going for a run..."
                        value={notes}
                        onChange={(e) => setNotes(e.target.value)}
                        className="w-full px-5 py-3 bg-[#0f172a]/50 text-white border border-white/10 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 outline-none transition-all resize-none placeholder:text-slate-500"
                    ></textarea>
                </div>
